...
public class WebRTCManager: NSObject, ObservableObject {
            ...
            peerConnection.setLocalDescription(sdp) { [weak self] error in
                guard let self = self, error == nil else {
                    self?.callbacks.onError(
                        "Failed to set local description: \(String(describing: error))", nil)
                    ...
                    do {
                        ...
                        case .openai:
                            let sessionConfig = SessionConfig(
                                ...
                                temperature: 0.6,  // You might want to make this configurable
                                instructions: systemInstructions,
                                tools: [:],  // Empty dictionary for tools since it's required by the struct
                                turn_detection_type: "server_vad"
                            )
                            let ephemeralKey = try await Utils.GetEphemeralKey(apiKey: apiKey, sessionConfig: sessionConfig, baseUrl: self.provider.baseURL)
                            print("[Outspeed] Finished sending request")
                            let answerSdp = try await self.fetchRemoteSDPOpenAI(
                                apiKey: apiKey, localSdp: localSdp)
                        ...
                        case .outspeed:
                            ...
                            let answerSdp = try await self.fetchRemoteSDPOutspeed(
                                    ephemeralKey: ephemeralKey, localSdp: localSdp)
                        }
                    } catch {
    ...
    public func sendMessage() {
        guard let dc = dataChannel,
            !outgoingMessage.trimmingCharacters(in: .whitespaces).isEmpty
        else {
            return
        }
    ...
    private func getEphemeralKeyOutspeed(apiKey: String) async throws -> String {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw NSError(
                domain: "WebRTCManager.getEphemeralKeyOutspeed",
                code: code,
    ...
    private func fetchRemoteSDPOpenAI(apiKey: String, localSdp: String) async throws -> String {
        ...
        print(
            "[Outspeed] Request Body: \(String(data: request.httpBody ?? Data(), encoding: .utf8) ?? "nil")"
        )
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            let code = (response as? HTTPURLResponse)?.statusCode ?? -1
            throw NSError(
                domain: "WebRTCManager.fetchRemoteSDPOpenAI",
                code: code,
                userInfo: [NSLocalizedDescriptionKey: "Invalid server response"])
        }
    ...
    private func fetchRemoteSDPOutspeed(ephemeralKey: String, localSdp: String) async throws {
        ...
        try await withCheckedThrowingContinuation {
            ...
            func receiveMessage() {
                webSocket.receive { [weak self] result in
                    guard let self = self else {
                        ...
                        continuation.resume(
                            throwing: NSError(
                                domain: "WebRTCManager.fetchRemoteSDPOutspeed", code: -2,
                                userInfo: [
                                    NSLocalizedDescriptionKey:
                                        "WebRTCManager deallocated during WebSocket operation"
                                ]))
                        return
                    }
                    ...
                    case .success(let message):
                        ...
                        case .string(let text):
                            ...
                            case "pong":
                                ...
                                guard
                                    let offerData = try? JSONSerialization.data(
                                        withJSONObject: offerMessagePayload),
                                    let offerString = String(data: offerData, encoding: .utf8)
                                else {
                                    continuation.resume(
                                        throwing: NSError(
                                            domain: "WebRTCManager.fetchRemoteSDPOutspeed",
                                            code: -1,
                                            userInfo: [
                                                NSLocalizedDescriptionKey:
                                                    "Failed to serialize offer message to string"
                                            ]))
                                    return
                                }
                                webSocket.send(.string(offerString)) { error in
                                    if let error {
                                        print(
                                            "[Outspeed][WebSocket] Failed to send offer: \(error)")
                                        continuation.resume(throwing: error)
                                    } else {
                                        print(
                                            "[Outspeed][WebSocket] Offer sent. Waiting for answer..."
                                        )
                                        receiveMessage()
                                    }
                                }
                            ...
                            case "candidate":
                                    ...
                                    let sdpMLineIndex = json["sdpMLineIndex"] as? Int
                                {
                                    ...
                                    self.peerConnection?.add(iceCandidate) { error in
                                        if let error = error {
                                            print(
                                                "[Outspeed][WebSocket] Failed to add ICE candidate: \(error)"
                                            )
                                        }
                                    }
                                    receiveMessage()
                            ...
                            default:
                                print(
                                    "[Outspeed][WebSocket] Unknown message type received: \(type)")
                                receiveMessage()
                            }
                        case .data(let data):
                            print(
                                "[Outspeed][WebSocket] Received binary data (unexpected): \(data as NSData)"
                            )
                            receiveMessage()
                        @unknown default:
            ...
            guard let pingData = try? JSONSerialization.data(withJSONObject: pingMessagePayload),
                ...
                continuation.resume(
                    throwing: NSError(
                        domain: "WebRTCManager.fetchRemoteSDPOutspeed", code: -1,
                        userInfo: [
                            NSLocalizedDescriptionKey: "Failed to serialize ping message to string"
                        ]))
                return
            }
            ...
            webSocket.send(.string(pingString)) { error in
                if let error {
                    ...
                    print(
                        "[Outspeed][WebSocket] Initial ping sent successfully. Waiting for pong...")
                    receiveMessage()
                }
            }
        }
